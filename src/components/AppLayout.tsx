    ...
    <div className="bg-background text-foreground min-h-screen flex">
      ...
      <aside className="w-20 lg:w-64 bg-card p-4 flex flex-col items-center lg:items-start transition-all duration-300">
        ...
        <nav className="flex flex-col gap-4 w-full">
            {navItems.map((item) => (
                <Button key={item.href} variant="ghost" className={cn("justify-center lg:justify-start gap-3", pathname === item.href ? "bg-secondary" : "")} asChild>
                    <Link href={item.href}>
                        <item.icon />
                        <span className="hidden lg:inline">{item.label}</span>
                        {item.new && <Badge variant="secondary" className="bg-accent/20 text-accent hidden lg:inline">Baru</Badge>}
                    </Link>
                </Button>
            ))}
        </nav>
